...
const approvedTokens: DesignToken[] = [
  { id: "brand-500", category: "color", name: "Brand Primary", value: "#A78BFA" },
...
describe("driftReport", () => {
  it("returns zero drift items when all hex values are approved", () => {
    const html = `
      <div style="color: #A78BFA; background: #FFFFFF;">
  ...
  it("flags off-brand hex values as drift items", () => {
    const html = `
      <div style="color: #FF0000; background: #A78BFA;">
  ...
  it("handles case-insensitive hex matching", () => {
    const html = `<div style="color: #a78bfa;">text</div>`;
